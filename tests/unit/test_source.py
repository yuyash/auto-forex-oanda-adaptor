...
def test_data_source_prices_uses_account_pricing_endpoint() -> None:
    gateway = Mock()
    mapper = Mock()
    tick = Tick(
        instrument=USD_JPY,
...
def test_data_source_candles_uses_account_candles_endpoint() -> None:
    gateway = Mock()
    mapper = Mock()
...
def test_data_source_stream_prices_yields_mapped_non_heartbeat_parts() -> None:
    gateway = Mock()
    mapper = Mock()
    tick = Tick(
        instrument=USD_JPY,
...
def test_data_source_close_closes_gateway_opener() -> None:
    opener = Mock()
    gateway = Mock()
