...
def test_broker_place_order_uses_order_mapper_and_market_gateway() -> None:
    gateway = Mock()
    order_mapper = Mock()
...
def test_broker_account_currency_is_cached() -> None:
    gateway = Mock()
    account_mapper = Mock()
    account_mapper.account_currency_from_response.return_value = Currency.of("USD")
    gateway.get_account_summary.return_value = FakeResponse(200, {"account": {"currency": "USD"}})
...
def test_broker_close_position_builds_oanda_side_request() -> None:
    gateway = Mock()
    order_mapper = Mock()
    position = Position(
        instrument=USD_JPY,
...
def test_broker_positions_uses_position_mapper(monkeypatch: pytest.MonkeyPatch) -> None:
    gateway = Mock()
    account_mapper = Mock()
    mapper = Mock()
    position = Position(
    ...
    mapper.positions_from_response.assert_called_once_with(gateway.list_open_positions.return_value)
...
def test_broker_order_mutation_results_return_metadata() -> None:
    gateway = Mock()
    ...
    assert (
        broker.set_order_client_extensions("100", client_id="client", tag="tag", comment="comment")[
            "lastTransactionID"
        ]
        == "10"
    )
...
def test_order_type_helper_maps_core_order_types() -> None:
