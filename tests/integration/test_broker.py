...
def test_broker_integrates_gateway_and_mappers_without_http() -> None:
    gateway = IntegrationGateway()
    broker = OandaBroker(account_id="001", gateway=cast(OandaGateway, gateway))

    order = broker.place_order(Order(instrument=USD_JPY, side=OrderSide.BUY, units=Decimal("1000")))
