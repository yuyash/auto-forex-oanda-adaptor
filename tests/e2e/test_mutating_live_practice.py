...
def test_practice_account_configuration_can_be_changed_and_restored(
    ...
    if original.alias is None:
        pytest.skip("Account alias is empty; skipping alias mutation because it cannot be restored")
...
def _far_buy_limit_order(
    ...
    tick = next(iter(provider.data_source.prices(instruments=(instrument,))))
